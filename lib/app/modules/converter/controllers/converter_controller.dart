...
class ConverterController extends GetxController {
  // 当前选择的类别
  final Rx<ConversionCategory?> selectedCategory = Rx<ConversionCategory?>(
    null,
  );
  ...
  void selectCategory(ConversionCategory category) {
    ...
    if (category.units.isNotEmpty) {
      ...
      toUnit.value = category.units.length > 1
          ? category.units[1]
          : category.units[0];
    }
  ...
  void inputDigit(String digit) {
    ...
    if (selectedCategory.value?.id == 'number_system') {
      ...
      if (!ConversionLogic.isValidNumberSystemInput(
        testValue,
        fromUnit.value?.id ?? 'decimal',
      )) {
        return;
      }
  ...
  void _calculate() {
    ...
    try {
      ...
      // 温度特殊处理
      if (selectedCategory.value?.isSpecial == true &&
          selectedCategory.value?.id == 'temperature') {
